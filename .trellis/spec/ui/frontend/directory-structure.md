...
`@repo/ui` 只放 Web 和 Admin 都会使用、且不包含业务规则的 React 组件。当前源码在 `packages/ui/src/`，已有 `button.tsx`、`card.tsx` 和 `code.tsx`。
...
只有 Web 和 Admin 都会用的通用组件才放这里，例如 button、card、dialog、input、sidebar。只有一个应用使用的组件先留在该应用。
