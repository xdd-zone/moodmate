...
- [x] Web 和 Admin 的 server/client 环境入口能分别接受合法配置，并对缺失、非法枚举和非法 URL 给出明确错误。
- [x] Web 首页不再包含 `NEXT_PUBLIC_API_BASE_URL ?? "http://localhost:6155"` 这类回退逻辑。
- [x] API 继续只从 `c.env` 读取 binding，非法 `APP_ENV` 不再静默变成 `development`。
- [x] `APP_ENV`、`API_BASE_URL`、`NEXT_PUBLIC_APP_ENV`、`NEXT_PUBLIC_API_BASE_URL` 和 `CORS_ORIGINS` 在 `turbo.json` 中按任务声明。
- [x] 三个应用都提供可提交的示例文件，真实本地文件仍被 Git 忽略，示例中没有 secret。
- [x] `wrangler.jsonc` 能表达 development、test、production 三种业务环境，且没有编造远端域名。
- [x] Web、Admin 复用 catalog 中的 Zod 版本，没有新增共享 env package。
- [x] `pnpm check-types`、`pnpm lint`、`pnpm format:check` 依次通过。
- [x] `pnpm --filter web build`、`pnpm --filter admin build` 通过；API 在提供本地变量后可启动，`GET /health` 返回对应 `APP_ENV`。
- [x] Web、Admin、API 的说明与最终变量名和本地配置方式一致。
